...
Ext.ux.Multiselect = Ext.extend(Ext.form.Field, {
    ...
    onRender : function (ct, position) {
        ...
        this.legendEl = fs.el.down("legend");
        if (this.legendEl) {
            this.legendEl.addClass("x-multiselect-legend");
        }
    ...
    onResize : function (w, h) {
         this.fs.setSize(w,h);
         this.view.setSize(Ext.isNumber(w) ? (w - 2) : w, Ext.isNumber(h) ? (h - ((this.legendEl ? this.legendEl.getHeight() : 0) + 2)) : h);
    },
    ...
    onViewClick : function (vw, index, node, e) {
        ...
        this.fireEvent('change', this, this.getValue(), this.oldValue);
    ...
    getIndexes : function () {
        return (this.view.getSelectedIndexes() || []).join(this.delimiter);
    },

    setValue : function (values, keepExisting, suppressEvent) {
        if (!this.rendered) {
            this.on("render", function () {this.setValue(values, keepExisting, suppressEvent);}, this, {single: true});
            return;
        }
        ...
        for (var i = 0; i < values.length; i++) {
			index = this.view.store.indexOf(this.view.store.queryBy(function (record) {
			    return record.get(this.valueField) == values[i];
			}, this).itemAt(0));
    ...
    setValueByIndex : function (indexes, keepExisting, suppressEvent) {
        if (!this.rendered) {
            this.on("render", function () {this.setValueByIndex(indexes, keepExisting, suppressEvent);}, this, {single: true});
            return;
        }
    ...
    */
    selectAll: function() {
        this.view.selectRange(0, this.store.getCount());
    },
    ...
    */
    deselectAll: function(suppressEvent) {
        this.view.clearSelections(suppressEvent);
    }
});
